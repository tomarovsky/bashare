...
if [[ $# -ne 3 ]]; then
    ...
    echo "for S in {1..18}; do $0 mzib.allsamples.filt.masked.auto.snp.plink.0.2.vcf.gz mzib.w1mb.s100kb.features.auto.num.bed $S > /dev/null 2>&1 & done"
    exit 1
fi
...
for CHR in $SCAFFOLDS; do
    ...
    cat ${REGION_FILE} | grep -P "^$CHR\t" | while read REG; do
        ...
        if plink --vcf ${CHR}.${REGION}.vcf --out ${CHR}.${REGION}.plink --make-bed --threads 1; then
            ...
            for SAMPLE in {1..33}; do
                echo -e "${REG}\tEMPTY\tEMPTY" >> ${SAMPLE}.${CHR}.admixture.concat.Q;
            done;
            rm *plink* *.vcf;
        conda deactivate;
        fi;
