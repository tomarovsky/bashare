...
if grep -qw "$SAMPLE" "$MALES"; then
    ...
    bedtools intersect -a <(zcat "$PER_BASE_BED_FILE") -b "$HEMI_REGIONS_BED" | gzip > "${SAMPLE}.hemi.tmp.per-base.bed.gz" &
    bedtools intersect -v -a <(zcat "$PER_BASE_BED_FILE") -b "$HEMI_REGIONS_BED" | gzip > "${SAMPLE}.diploid.tmp.per-base.bed.gz" &
    ...
    cat ${SAMPLE}.diploid.per-base.mask.bed ${SAMPLE}.hemi.per-base.mask.bed | sort -S 20G -k1,1 -k2,2n > "${PER_BASE_BED_FILE%.*.*}.max250.min33.bed"
