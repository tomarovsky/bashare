...
if [[ $without_blocked_users || $without_sbinnologin_users ]]; then
	...
	cat ${output} | while read line; do
		...
		if ${without_blocked_users}; then
			if [[ -n $(sudo passwd --status ${username} | grep "Password locked") ]]; then
				echo ${username} >> temp;
			fi
		fi
		if ${without_sbinnologin_users}; then
			if [[ -n $(getent passwd ${username} | grep "/sbin/nologin") ]]; then
				echo ${username} >> temp;
			fi
		fi
	done;
	for name in $(sort -u temp); do
		grep -v -P "^${name}\t" ${output} > t && mv t ${output};
	done;
	rm temp;
fi
